...
function loadContextMenus(forcePref) {
    ...
    for (let window of windowUtils.windows(null, {includePrivate:true})) {
        if (windowUtils.isBrowser(window)) {
            (function(w) {
                let menu = w.document.getElementById("tabContextMenu"),
                    preExistingItem = w.document.getElementById(menuId);
                ...
                if (!preExistingItem && pref) {
                    let newMenuItem = w.document.createElementNS(xulNs, "menuitem");
                    ...
                    newMenuItem.addEventListener('command', function() {
                        openTabOrigin(tabsUtils.getTabId(w.TabContextMenu.contextTab));
                    });
